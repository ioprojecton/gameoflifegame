...
byte cells[HEIGHT][WIDTH];
byte newCells[HEIGHT][WIDTH];
int sum1 = 0;
...
void setup() {
  ...
  for (byte row = 0; row < WIDTH; row++) for (byte col = 0; col < HEIGHT; cells[row][col++] = random(0, 2));
...
int checkSum() {
  int sum = 0;
  for (byte row = 0; row < WIDTH; row++) for (byte col = 0; col < HEIGHT; cells[row][col++]) sum += cells[row][col];
  return sum;
}
...
void writeNextGeneration() {
  for (byte row = 0; row < WIDTH; row++) for (byte col = 0; col < HEIGHT; col++) newCells[row][col] = cells[row][col];
